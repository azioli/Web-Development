<?php
	...
	if(isset($_GET['keyword'])){
		...
		if($_GET['keyword']=='Place' && isset($_GET['lat']) && isset($_GET['lon'])){
			$url="https://graph.facebook.com/v2.8/search?q=".urlencode($_GET['keyword'])."&type=place&fields=id,name,picture.width(700).height(700)&center=".urlencode($_GET['lat']).",".urlencode($_GET['lon'])."&access_token=".urlencode(FB_TOKEN);
			...
			if(isset($type_arr[$_GET['type']])){
				$result = file_get_contents("https://graph.facebook.com/v2.8/search?q=".urlencode($_GET['keyword'])."&type=".$type_arr[$_GET['type']]."&fields=id,name,picture.width(700).height(700)&access_token=".urlencode(FB_TOKEN));
				if(!$result){
					$response = array(
						'status' => false,
						'message' => 'An error occurred!'
